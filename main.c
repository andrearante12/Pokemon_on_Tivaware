 ...
 *
 * Headers generated by:  python3 gen_scene.py
 */
...
/* ── ADC + button init (joystick on PE3=AIN0/VRx, PE2=AIN1/VRy, PB0=SW) ── */
...
/* ── Cursor glyphs: '>' and ' ' in 5-row pixel font (11 display cols each) ─
 *
 *  '>' font rows: ['█    ','██   ','███  ','██   ','█    ']
 *  gap between chars: 1 space
 *  ' ' font rows: ['     ','     ','     ','     ','     ']
 *
 *  Combined '> ' per row  (5 + 1 gap + 5 = 11 display cols):
 *    row 0: █ + 10 spaces
 *    row 1: ██ + 9 spaces
 *    row 2: ███ + 8 spaces
 *    row 3: ██ + 9 spaces
 *    row 4: █ + 10 spaces
 */
#define BL "\xe2\x96\x88"   /* █ UTF-8 filled block */

static const char * const CURSOR_SEL[5] = {
    BL "          ",      /* row 0: 1 block + 10 spaces */
    BL BL "         ",    /* row 1: 2 blocks + 9 spaces */
    BL BL BL "        ",  /* row 2: 3 blocks + 8 spaces */
    BL BL "         ",    /* row 3: 2 blocks + 9 spaces */
    BL "          ",      /* row 4: 1 block + 10 spaces */
};

static const char * const CURSOR_EMPTY[5] = {
    "           ",   /* 11 spaces */
    "           ",
    "           ",
    "           ",
...
/*
 * Redraws the 3-row HP bar content (the █/░ fill) for enemy (is_player=0)
 * or player (is_player=1).  Does not touch the border or label rows.
 */
void update_hp_bar(int is_player, int hp, int max_hp)
{
    ...
    for (r = 0; r < 3; r++) {
        ansi_goto(row + r, col);
        for (i = 0; i < filled; i++) uart_puts(BL);
        for (i = 0; i < empty;  i++) uart_puts("\xe2\x96\x91");  /* ░ */
    }
}

/*
 * Moves the '>' cursor from old_sel to new_sel (0-3).
 * Only the 11-col cursor glyph at each move position is rewritten.
 */
...
/*
 * Writes a full move-slot label (5 rows, padded to MOVE_LABEL_W) at slot.
 * Padding ensures any previously wider label is fully overwritten.
 */
...
/*
 * Replaces the dialog text area with a new pre-rendered message.
 * 'rows' points to a null-terminated array of 11 strings (5 + empty + 5)
 * as emitted by gen_scene.py into scene_pos.h (e.g. DIALOG_IDLE).
 */
...
/*
 * Replaces a sprite with a new frame (for animations, Phase 3).
 * 'sprite' is a null-terminated array of strings matching the original
 * sprite dimensions.
 */
...
void Timer0IntHandler(void)
{
    ...
    const note_t *track = PLAYLIST[g_track_idx];
    ...
    if (g_ticks_left == 0) {
        ...
        if (track[g_note_idx].freq_hz == 0 && track[g_note_idx].dur_ms == 0) {
            g_note_idx  = 0;
            g_track_idx = (g_track_idx + 1) % PLAYLIST_LEN;
            track       = PLAYLIST[g_track_idx];
        }
    ...
    while (1) {
        ...
        if (state == STATE_MENU) {
            ...
            if (sw1_pressed()) {
                ...
                /* BAG / POKEMON / RUN: not yet implemented */
                while (GPIOPinRead(GPIO_PORTF_BASE, GPIO_PIN_4) == 0) {}
            }
        ...
        /* ── Player executes chosen move ── */
            ...
            if (move_choice == 0) {          /* Water Gun */
                ...
                damage = 15 + 5 * defense_drops;
                delay_sec(2);
                enemy_hp -= damage;
                if (enemy_hp < 0) enemy_hp = 0;
                update_hp_bar(0, enemy_hp, 45);
                delay_sec(1);
